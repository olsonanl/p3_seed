...
p3-inAandB File1 File2 > intersection
...
Usage: p3-inAandB File1 File2 > intersection
...
my($f1,$f2);
my $usage = "usage: p3-inAandB File1 File2 > intersection";
...
my %keep;
foreach my $x (grep { $f2H{$_} } `tail -n+2 $f1`)
{
    $keep{$x} = 1;
}
